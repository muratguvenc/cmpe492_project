...
function drawCircleDiffusion(x,y,r) {
...
function drawCellCircleDiffusion(x,y,r) {
...
function drawScalerDiffusion(multiple) {
...
function drawInputStreamDiffusion(stream){
...
function receiveCountDiffusion(color){
	if(color == 1){
...
function receiveMoleculeNumberDiffusion(countMolecules){
	recPaper.text(565, 365, countMolecules).attr({"font-size": 12,"font-weight": "bold"});
}

function drawEmptyClockDiffusion(output){
...
function showStatisticsDiffusion(totalTime, totalArrive, correctData, length){
	var a = new Array();
	...
	a[2] = paper.text(260, -190, '\u2022 Total # molecules arrived to receiver : '+totalArrive).attr({"font-size": 14,"font-weight": "bold", "text-anchor":"start"});
	a[3] = paper.text(260, -170, '\u2022 Avg. # molecules arrived to receiver in').attr({"font-size": 14,"font-weight": "bold", "text-anchor":"start"});
	a[4] = paper.text(360, -150, 'a symbol time: '+(Math.round(totalArrive/length*100))/100).attr({"font-size": 14,"font-weight": "bold", "text-anchor":"start"});
	a[5] = paper.text(260, -130, '\u2022 Success percentage of data ').attr({"font-size": 14,"font-weight": "bold", "text-anchor":"start"});
	a[6] = paper.text(360, -110, 'transmission : '+(Math.round(correctData/length*100))/100).attr({"font-size": 14,"font-weight": "bold", "text-anchor":"start"});
...
function clearPaperDiffusion() {
    paper.clear();
}

function clearRecPaperDiffusion() {
    recPaper.clear();
}

function clearCoverDiffusion() {
	coverPaper.clear();
}

function clearClockPaperDiffusion() {
	clockPaper.clear();
}
